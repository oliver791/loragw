...
// version modifiée qui envoie en continue dès le démarrage avec 4 paramètres CLI: power, SF, fréquence, et taille du payload
...
static uint32_t csv_pkt_count = 0;
...
static const char* ftype_to_str(uint8_t ftype) {
    switch (ftype) {
...
static void csv_log_open(void) {
    csv_log_file = fopen("gateway_log.csv", "w");
    if (csv_log_file == NULL) {
        MSG("ERROR: impossible d'ouvrir gateway_log.csv\n");
        return;
    }
    fprintf(csv_log_file,
        "pkt_num,timestamp,direction,freq_mhz,sf,rf_power,rssi,snr,size,crc,frame_type,dev_addr,payload_hex\n");
    fflush(csv_log_file);
    MSG("INFO: gateway_log.csv ouvert\n");
}


static void csv_log_packet(struct lgw_pkt_rx_s *p) {
    if (csv_log_file == NULL || p == NULL) return;

    struct timespec now;
    clock_gettime(CLOCK_REALTIME, &now);
    struct tm tm_info;
    gmtime_r(&now.tv_sec, &tm_info);
    char time_str[64];
    ...
    switch (p->status) {
        ...
        default:           crc_str = "?"; break;
    }

    char ftype_str[20] = "N/A";
    char dev_addr_str[12] = "N/A";

    if (p->size >= 1) {
        uint8_t ftype = (p->payload[0] >> 5) & 0x07;
        snprintf(ftype_str, sizeof(ftype_str), "%s", ftype_to_str(ftype));

        if ((ftype == 2 || ftype == 4) && p->size >= 5) {
            uint32_t dev_addr = (uint32_t)p->payload[1]
                              | ((uint32_t)p->payload[2] << 8)
                              | ((uint32_t)p->payload[3] << 16)
                              | ((uint32_t)p->payload[4] << 24);
            snprintf(dev_addr_str, sizeof(dev_addr_str), "%08X", dev_addr);
        }
        else if (ftype == 0 && p->size >= 18) {
            snprintf(dev_addr_str, sizeof(dev_addr_str), "%02X%02X%02X%02X",
                p->payload[12], p->payload[11], p->payload[10], p->payload[9]);
        }
    }

    char payload_hex[512] = "";
    int hex_len = (p->size > 255) ? 255 : p->size;
    for (int j = 0; j < hex_len; j++) {
        sprintf(payload_hex + j * 2, "%02X", p->payload[j]);
    }

    pthread_mutex_lock(&csv_mutex);
    csv_pkt_count++;
    fprintf(csv_log_file,
        "%u,%s.%03ld,UP,%.3f,%s,,%.1f,%.1f,%u,%s,%s,%s,%s\n",
        csv_pkt_count,
        time_str,
        now.tv_nsec / 1000000,
        (double)p->freq_hz / 1e6,
        sf_to_str(p->datarate),
        /* rf_power vide pour UP */
        p->rssi,
        p->snr,
        p->size,
        crc_str,
...
static void csv_log_tx_packet(struct lgw_pkt_tx_s *p, const char *dl_type) {
    if (csv_log_file == NULL || p == NULL) return;
...
{
    /* ============================================================ */
    /* === PARSE CLI: power  sf  freq_hz  payload_size          === */
    /* === Usage: ./lora_pkt_fwd [power] [sf] [freq] [size]     === */
    /* ============================================================ */
    if (argc >= 2) {
        int pw = atoi(argv[1]);
        if (pw >= 0 && pw <= 27) {
    ...
    if (argc >= 4) {
        uint32_t fhz = (uint32_t)strtoul(argv[3], NULL, 10);
    ...
    MSG("INFO: Usage: ./lora_pkt_fwd [rf_power] [sf] [freq_hz] [payload_size]\n");
    ...
    if (csv_log_file != NULL) {
        fclose(csv_log_file);
        MSG("INFO: gateway_log.csv ferme (%u paquets)\n", csv_pkt_count);
    }
...
/* -------------------------------------------------------------------------- */
/* --- THREAD 2: CONTINUOUS RANDOM DOWNLINK TRANSMISSION -------------------- */
...
void thread_down(void) {
    ...
    while (!exit_sig && !quit_sig) {
        ...
        if (send_result == LGW_HAL_SUCCESS) {
            ...
            csv_log_tx_packet(&txpkt_auto, "AUTO_DL");
        } else {
